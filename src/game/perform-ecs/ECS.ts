...
export class ECS {
    ...
    public removeComponentsFromEntity(entity: any, components: ComponentConstructor[] | ComponentConstructor): void {
    ...
    public addComponentsToEntity(entity: Entity, components: ComponentInitializator[]): void {
        ...
        for (const component of components) {
            const inst = this._getComponentInstance(component.component);
            inst.reset(entity as any, ...(component.args ?? []) );
        }
...
type createEntityFunc = <T extends ComponentInitializator,
    ...
    & ComponentOf<T7> & ComponentOf<T8> & ComponentOf<T9> & ComponentOf<T10> & {id: number} ;
